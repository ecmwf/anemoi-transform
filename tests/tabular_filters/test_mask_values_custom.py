...
def test_mask_values_custom():
    config = {
        "col1": "col1 >= 2",
    }
    df = pd.DataFrame({"col1": [0, 1, 2, 3], "col2": [3, 4, 5, 6]})
    mask_values_custom = create_filter("mask_values_custom", **config)
    result = mask_values_custom(df.copy())
...
def test_mask_values_custom_missing_column():
    config = {
        "col1": "col1 >= 2",
    }
    # col1 missing
