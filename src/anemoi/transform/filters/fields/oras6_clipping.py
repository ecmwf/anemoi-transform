...
class Oras6Clipping(MatchingFieldsFilter):
    ...
    def forward_transform(
        ...
        yield self.new_field_from_numpy(siconc_np, template=siconc, **{"parameter.variable": self.siconc})
        yield self.new_field_from_numpy(siue_np, template=siue, **{"parameter.variable": self.siue})
        yield self.new_field_from_numpy(sivn_np, template=sivn, **{"parameter.variable": self.sivn})
        yield self.new_field_from_numpy(icesalt_np, template=icesalt, **{"parameter.variable": self.icesalt})
        yield self.new_field_from_numpy(sihc_np, template=sihc, **{"parameter.variable": self.sihc})
        yield self.new_field_from_numpy(snhc_np, template=snhc, **{"parameter.variable": self.snhc})
        yield self.new_field_from_numpy(sipf_np, template=sipf, **{"parameter.variable": self.sipf})
        yield self.new_field_from_numpy(sitemptop_np, template=sitemptop, **{"parameter.variable": self.sitemptop})
        yield self.new_field_from_numpy(sntemp_np, template=sntemp, **{"parameter.variable": self.sntemp})
        yield self.new_field_from_numpy(snvol_np, template=snvol, **{"parameter.variable": self.snvol})
        yield self.new_field_from_numpy(sivol_np, template=sivol, **{"parameter.variable": self.sivol})
        yield self.new_field_from_numpy(sialb_np, template=sialb, **{"parameter.variable": self.sialb})
        yield self.new_field_from_numpy(vasit_np, template=vasit, **{"parameter.variable": self.vasit})
        yield self.new_field_from_numpy(tos_np, template=tos, **{"parameter.variable": self.tos})
