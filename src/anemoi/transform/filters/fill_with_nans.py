    ...
    def backward(self, fields: ekd.FieldList) -> ekd.FieldList:
        ...
        for idx_input_lat in range(nb_lats_input):
            ...
            list_idx = (
                ...
                + list_idx_output_lon[nb_lon_before_lat_in_input : nb_lon_before_lat_in_input + nb_lon_by_lat_in_input]
            )
            print("idx lat dans output_data qu'on va écrire", idx_output_lat)
            print(
                "idx lon dans output_data qu'on va écrire",
                list_idx_output_lon[nb_lon_before_lat_in_input : nb_lon_before_lat_in_input + nb_lon_by_lat_in_input],
            )
            print(
                "idx dans input data qu'on préleve",
                nb_lon_before_lat_in_input,
                nb_lon_before_lat_in_input + nb_lon_by_lat_in_input,
            )
