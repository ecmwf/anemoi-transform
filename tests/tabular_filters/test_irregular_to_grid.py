...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {
                "window_date_column": "window_date",
                "time_freq": "6h",
            ...
            pd.DataFrame(
                {
                    ...
                    "window_date": pd.Series(dtype="datetime64[ns]"),
...
def test_irregular_to_grid(mock_define_grid, small_grid, config, df, expected_arrays):
    ...
    expected_times = list(pd.to_datetime(df["window_date"].unique()))
    columns = config["columns"]
    assert len(result) == len(expected_times) * len(columns)

    for field in result:
        param = field.parameter.variable()
        vdt = pd.Timestamp(field.time.valid_datetime())
        ...
        t_idx = expected_times.index(vdt)
    ...
    df = pd.DataFrame(
        {
            ...
            "window_date": pd.to_datetime(["2023-01-01 06:00"]),
            "spatial_index": [0],
            "temperature": [10.0],
        }
    )
